...
export function RequestFormsTable() {
    ...
    const { data: requestsData, isLoading, error, refetch } = useGetRequestsQuery({ page });
    const requests = requestsData?.data || [];

    const currentPage = requestsData?.data?.current_page ?? page;
    const totalPages = requestsData?.data?.last_page ?? 1;
